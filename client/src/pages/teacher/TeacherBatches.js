...
import { 
  FiFolder, FiUsers, FiClock, FiCheckSquare, FiSearch, FiPhone, 
  FiUser, FiMail, FiChevronDown, FiChevronRight, FiCalendar, FiArrowRight
} from 'react-icons/fi';
    ...
    api.get('/teachers/my-data')
      .then(res => {
        ...
        if (res.data?.batches?.length > 0) {
          setExpandedBatchId(res.data.batches[0].id); // Expand first batch by default
        }
      })
      .catch(console.error)
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-80 gap-3">
        ...
        <span className="text-slate-500 text-sm font-medium">Loading running batches...</span>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-slate-900 flex items-center gap-2">
            <FiFolder className="text-amber-600" /> Running Batch Folders
          </h1>
          <p className="text-slate-500 text-sm">
            Browse your assigned batches, student rosters, and parent contact details
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Link 
            ...
            className="btn-primary py-2 px-4 text-sm flex items-center gap-1.5 shadow-sm"
          >
            <FiCheckSquare size={16} /> Take Daily Attendance
          </Link>
        </div>
      </div>
            ...
            >
              ...
              <div 
                ...
                className={`p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 cursor-pointer select-none transition-colors ${
                  isOpen ? 'bg-amber-50/50 border-b border-amber-100' : 'hover:bg-slate-50'
                }`}
              >
                <div className="flex items-center gap-3.5">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold transition-all ${
                    isOpen ? 'bg-amber-500 text-white shadow-md' : 'bg-amber-100 text-amber-700'
                  }`}>
                    <FiFolder />
                  </div>
                  <div>
                    <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                      ...
                      <span className="badge bg-slate-100 text-slate-700 text-xs font-semibold">
                        {batch.course?.name || 'Commerce'}
                      </span>
                    </h3>
                    <div className="flex items-center gap-3 text-xs text-slate-500 mt-1">
                      <span className="flex items-center gap-1"><FiClock size={12} /> {batch.timing || 'Regular Timing'}</span>
                      <span>•</span>
                ...
                <div className="flex items-center gap-3 self-end sm:self-auto">
                  <Link
                    to={`/teacher/attendance?batchId=${batch.id}`}
                    onClick={e => e.stopPropagation()}
                    className="btn-secondary text-xs py-1.5 px-3 rounded-lg flex items-center gap-1 hover:bg-amber-500 hover:text-white"
                  >
                    <FiCheckSquare size={13} /> Mark Attendance
                  </Link>
                  ...
                  {students.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {students.map(st => (
                        <div 
                          key={st.id} 
                          className="bg-white rounded-xl p-4 border border-slate-200 shadow-xs hover:border-amber-300 transition-all flex flex-col justify-between"
                        >
                          <div className="space-y-2">
                            ...
                            <div className="bg-slate-50 p-2.5 rounded-lg text-xs space-y-1 border border-slate-100">
                              ...
                              <div className="flex items-center justify-between text-slate-600">
                                <span className="text-slate-400">Parent WhatsApp:</span>
                          ...
                          <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between gap-2">
                            {(st.parentPhone || st.phone) && (
                              <a 
                                href={`https://api.whatsapp.com/send?phone=91${(st.parentPhone || st.phone).replace(/[^0-9]/g, '')}&text=${encodeURIComponent(`Namaste ${st.parentName || 'Parent'}, regarding ${st.name}'s performance in ${batch.name} class - D's Education`)}`}
                                target="_blank"
                                rel="noreferrer"
        ...
        {filteredBatches.length === 0 && (
          <div className="card p-12 text-center bg-white">
            ...
            <p className="text-slate-500 text-sm mt-1">
              {searchQuery ? 'No batches match your search query.' : 'You do not have any assigned batches yet.'}
            </p>
          </div>
        )}
      </div>
